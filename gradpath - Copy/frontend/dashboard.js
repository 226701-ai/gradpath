...
let employabilityChartInstance = null;
...
async function loadDashboard() {
    ...
    animateValue(
        ...
        data.average_employability_score || 0
    );

    /* AVERAGE EMPLOYABILITY PROGRESS */
    const avgScore = Math.min(
        Number(data.average_employability_score || 0),
        100
    );

    const avgProgress =
        document.getElementById("averageEmployabilityProgress");

    if (avgProgress) {
        avgProgress.style.width = avgScore + "%";
        avgProgress.textContent = avgScore + "%";
    }
    ...
    if (employabilityChartInstance) employabilityChartInstance.destroy();
    ...
    courseEnrolmentChartInstance = new Chart(
        document.getElementById("courseEnrolmentChart"),
        {
            type: "bar",
            data: {
                labels: (data.course_enrolment_chart || []).map(
                    i => i.course_code
                ),
                datasets: [{
                    label: "Enrolled Students",
                    data: (data.course_enrolment_chart || []).map(
                        i => i.total
                    ),
    ...
    /* EMPLOYABILITY LEVEL CHART */
    employabilityChartInstance = new Chart(
        document.getElementById("employabilityChart"),
        {
            type: "doughnut",
            data: {
                labels: (data.employability_level_chart || []).map(
                    i => i.level
                ),
                datasets: [{
                    data: (data.employability_level_chart || []).map(
                        i => i.total
                    ),
                    backgroundColor: [
                        "#27ae60",
