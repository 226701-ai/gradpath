...
.addEventListener("submit", async (e) => {
    ...
    const studentActivity = {
        ...
        role: document.getElementById("role").value // <--- ADD THIS LINE
    };
...
function displayStudentActivities(records) {
    ...
    if (!records.length) {
        list.innerHTML = `
            <tr>
                <td colspan="6" style="text-align:center;"> No student activity record found
                </td>
            </tr>
        `;
        return;
    ...
    records.forEach(record => {
        list.innerHTML += `
            <tr>
                ...
                <td>${record.role || 'N/A'}</td> <td>${record.score}</td>
